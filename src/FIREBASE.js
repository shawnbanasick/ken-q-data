...
(function (FIREBASE, QAV, undefined) {
    ...
    FIREBASE.stageJsonData = function () {
        ...
        // bail out of function if no Q-sort statements added to DOM
        if (array1 === "error") {
            return;
        }
    ...
    FIREBASE.processStatementInput = function (statementInput) {
        ...
        if (arr.length < 2) {
            console.log("no statements uploaded for JSON");
            var errorPanel = $("#genericErrorModal .errorPanel");
            errorPanel.empty();
            errorPanel.append("<p>Please add Q-sort statements before staging JSON data</p><br>");
